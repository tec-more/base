...
Tukecx.DataTableAjax = function ($) {
    ...
    var initEachItem = function ($table, options) {
        ...
        var dataTableHelper = new Tukecx.DataTable();
        ...
        dataTableHelper.getTableWrapper().on('confirmed.bs.confirmation', '.table-group-action-submit', function (e) {
            ...
            if (action.val() != "" && dataTableHelper.getSelectedRowsCount() > 0) {
                ...
                Tukecx.showNotification('Please select an action', 'danger');
            } else if (dataTableHelper.getSelectedRowsCount() === 0) {
                Tukecx.showNotification('No record selected', 'warning');
            }
        });
                ...
                complete: function (data) {
                    ...
                    if (typeof data.responseJSON !== 'undefined') {
                        if (data.responseJSON.error) {
                            Tukecx.showNotification(data.responseJSON.messages, 'danger');
                        }
                        else {
                            Tukecx.showNotification(data.responseJSON.messages, 'success');
                        }
                    }
                    else {
                        Tukecx.showNotification('Some error occurred. View console log for more information', 'danger');
                    }
                    dataTableHelper.getDataTable().ajax.reload();
                }
            });
