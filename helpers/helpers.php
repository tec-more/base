...
if (!function_exists('get_cms_version')) {
    ...
    function get_cms_version()
    {
        try {
            ...
            return array_get($packages->where('name', '=', 'tec-more/base')->first(), 'version');
...
if (!function_exists('load_module_helpers')) {
    ...
    {
        \Tukecx\Base\Core\Support\Helper::loadModuleHelpers($dir);
    }
}
...
if (!function_exists('is_in_dashboard')) {
    ...
    function is_in_dashboard()
    {
        ...
        if ($segment === config('tukecx.admin_route', 'admincp')) {
            return true;
        }
